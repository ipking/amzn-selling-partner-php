...
while(1){
	...
	if(strtoupper($payload['processingStatus']) == 'DONE'){
		break;
	}
}
...
$response = $feedClient->getReportDocument($response['payload']['reportDocumentId']);
print_r($response);

$payload = $response['payload'];

$result = (new \SellingPartner\Helper\Report())->downloadProcessingReport($payload);
print_r($result);
